...
public class FileHandler {
    // 파일 저장 + DB에 들어갈 List 생성
    private final String UP_DIR = "D:\\web_260316_omr\\_myProject\\_java\\_fileUpload";
    ...
    public List<FileDTO> uploadFile(MultipartFile[] files){
        ...
        for(MultipartFile file : files){
            ...
            log.info("file.getOriginalFilename >> {}", file.getOriginalFilename()); // 실제 파일명
            log.info("file.getName >> {}", file.getName()); // files
            log.info("file.getContentType >> {}", file.getContentType()); // image/jpeg
